...
void good_thomas(float** xData, float** X, int N, int columns, int rows){
    /* Reshape */
    float*** x2 = (float***)malloc(sizeof(float**) * rows);
    float*** x2_0 = (float***)malloc(sizeof(float**) * rows);
    for(int x = 0; x < rows; x++){
    ...
    int y = 0;
    for(int i = 0; i < N; i++){
        x2[x][y][REAL]=xData[i][REAL];
        x2[x][y][IMAG]=xData[i][IMAG];
        x = (x+1)%rows;
        y = (y+1)%columns;
    }
    ...
    /* Rearrage matrix for column DTFT */
    for(int x = 0; x < rows; x++){
        for(int y = 0; y < columns; y++){
            x2_1[y][x][REAL] = x2_0[x][y][REAL];
            x2_1[y][x][IMAG] = x2_0[x][y][IMAG];
        }
    }
    /* DTFT columns */
    for(int y = 0; y < columns; y++){
    ...
    for(int x = 0; x < rows; x++){
        for(int y = 0; y < columns; y++){
            int k = (columns*x+rows*y)%N;
            X[k][REAL] = x2_2[y][x][REAL];
            X[k][IMAG] = x2_2[y][x][IMAG];
        }
    }
